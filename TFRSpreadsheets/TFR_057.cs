...
namespace TFRSpreadsheets
{
    public partial class TFRCommon
    {
        public string E57_TUI_030366()
        {
            ...
            bool WithStaffTotal = mReport.BooleanOption1;
            Statement = new Totals("Statement", $"INVOICE SUMMARY {mReport.Date1To:yyyyMMdd}","");

            xlWorkSheet.FreezePanes(1, 0);
            xlWorkSheet.SetColumnStyle(2, xlStyleDate);
            xlWorkSheet.SetColumnStyle(5, xlStyleInteger);
            xlWorkSheet.SetColumnStyle(8, xlStyleDate);
            xlWorkSheet.SetColumnStyle(12, 15, xlStyleDecimal);
            RowCounter = 2;
            ...
            xlWorkSheet.SetCellStyle(RowCounter, 1, RowCounter, LastColumn, xlStyleHeader);
            RowCounter++;
            ...
            for (int i = 0; i < mdsDataSet.Tables[0].Rows.Count; i++)
            {
                ...
                if (Client == null || Client.Key != clientkey)
                {
                    ...
                    xlWorkSheet.SetCellStyle(RowCounter, 1, RowCounter, LastColumn, xlStyleTotalsYellow);
                ...
                if (WithStaffTotal) Staff.Add(taxes, facevalue, discount, netpayable);
        ...
        int AddTotalLine(int rowcounter, Totals totalline,bool highlight)
        {
            if (totalline != null)
            {
                ...
                if (highlight)
                {
                    xlWorkSheet.SetCellStyle(rowcounter, 1, rowcounter, 15, xlStyleTotalsYellow);
                }
                else
                {
                    xlWorkSheet.SetCellStyle(rowcounter, 1, rowcounter, 15, xlStyleTotals);
                }
                rowcounter++;
            }
